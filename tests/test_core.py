...
from pytortilla.core import read_tortilla_metadata_local, read_tortilla_metadata_online, compile_local, compile_online
...
class TestReadTortillaMetadataLocal:
    ...
    def test_valid_file(self):
        ...
        with patch("builtins.open", mock_open(read_data=mock_file_content)) as mock_file:
            ...
            result["tortilla:file_format"] = result["tortilla:file_format"].str.strip("\x00")
    ...
    def test_invalid_magic_bytes(self):
        ...
        with patch("builtins.open", mock_open(read_data=mock_file_content)), pytest.raises(ValueError):
    ...
    def test_invalid_footer_content(self):
        ...
        with patch("builtins.open", mock_open(read_data=mock_file_content)), pytest.raises(json.JSONDecodeError):
            read_tortilla_metadata_local("dummy.tortilla")
...
class TestReadTortillaMetadataOnline:
    ...
    def test_read_tortilla_metadata_online_empty_footer(self, requests_mock):
        ...
        requests_mock.get(mock_url, json={}, headers={"Range": f"bytes={footer_offset}-{footer_offset + footer_length - 1}"})
    ...
    def test_read_tortilla_metadata_online_footer_length_mismatch(self, requests_mock):
        ...
        mock_bytes = (
            b"#y" +
            struct.pack("<Q", footer_offset) +
            struct.pack("<Q", 0) +  # Mismatch in footer length
            b"TortillaFormat".ljust(32, b"\x00")
        )
        requests_mock.get(mock_url, content=mock_bytes)

        # Simulating an incorrect footer range request
        requests_mock.get(mock_url, json={}, headers={"Range": f"bytes={footer_offset}-{footer_offset - 1}"})
    ...
    def test_read_tortilla_metadata_online_zero_footer_length(self, requests_mock):
        ...
        requests_mock.get(mock_url, content=b'{}', headers={"Range": f"bytes={footer_offset}-{footer_offset - 1}"})
...
class TestCompileLocal:
    ...
    def test_compile_local_no_file(self, mock_get, mock_open, sample_dataset):
        ...
        mock_get.return_value.__enter__.return_value.content = b""  # No content retrieved
        ...
        with pytest.raises(ValueError):
            compile_local(sample_dataset, output_file, chunk_size=50, nworkers=2, quiet=False)
    ...
    def test_compile_local_writing_no_concurrency(self, mock_mmap, mock_open):
        ...
        compile_local(sample_dataset, output_file, chunk_size=50, nworkers=1, quiet=True)
        ...
        mock_open.return_value.__enter__.return_value.truncate.assert_called_once_with(391)  # Verify file size after writing
        mock_open.assert_any_call(output_file, 'r+b')  # Ensure file is reopened for reading/writing
...
class TestCompileOnline:
    ...
    def sample_dataset(self):
        ...
        data = {
            "tortilla:subfile": ["https://example.com/file", "https://example.com/file2", "https://example.com/file3"],
    ...
    @patch("builtins.open", new_callable=MagicMock)
    def test_compile_online_resume_download(self, mock_open, mock_stat, mock_exists, mock_get, sample_dataset):
        #Simulates resuming a download when part of the file already exists

        mock_exists.return_value = True  # Indicates that the file already partially exists
        ...
        result = compile_online(sample_dataset, "output_file.tortilla", chunk_size=1024, quiet=False)
    ...
    def test_compile_online_no_download_when_complete(self, mock_stat, mock_exists):
        ...
        result = compile_online(sample_dataset, "output_file.tortilla", chunk_size=1024, quiet=True)
    ...
    @patch("pathlib.Path.stat")
    @patch("requests.get")
    def test_compile_online_resume_download_message(self, mock_get, mock_stat, mock_exists):
        #Verifies that an appropriate message is shown when resuming a partial download.
        
        mock_exists.return_value = True  # File exists
        mock_stat.return_value.st_size = 5050  # File partially downloaded up to 5050 bytes
        ...
        with patch("builtins.print") as mock_print:
            compile_online(sample_dataset, "output_file.tortilla", chunk_size=1024, quiet=False)
    ...
    def test_compile_online_fallback_total_bytes(self, mock_get):
        ...
        compile_online(sample_dataset, "output_file.tortilla", chunk_size=1024, quiet=True)
